...
| `0011_platform_tables.sql` | `players`, `sessions`, `game_sessions`, `leaderboards`, `matches` 테이블 + 기존 scores/analytics 백필 (Sprint 11) — **⚠️ Supabase SQL Editor에서 수동 실행 필요** |
...
**예고된 다음 스프린트 (PM 확정 로드맵, 2026-07-24)** — 상세는 `docs/sprint-11-plan.md`:
- **Sprint 11 (진행 중)**: Operations & SEO — `/admin` 운영 대시보드, Analytics 이벤트 수집, SEO/manifest/Search Console, Share, CLS/Lighthouse 95+ 목표. **신규 게임 없음.**
- **Sprint 12**: Cloud Save + Login (Google/Apple/Discord/GitHub OAuth), Cross-Device 동기화
- **Sprint 13**: Community
- **Sprint 14**: AI Recommendation + Home Tab UX 개편
- **Sprint 15**: Tournament / Replay (예비)
- **Sprint 16**: Retro Game Collection (10~15종, 게임성 참고 오리지널 구현)

> **로드맵 변경 이력:** HANDOFF 최초 작성 시 Sprint 11 = Cloud Save/OAuth로 적혀 있었으나, PM이 Sprint 10 완료 후 **플랫폼 운영·SEO 우선**으로 방향 전환. Cloud Save/Login은 Sprint 12로 연기.
