...
function statusMessage(state: ChessState, mode: GameMode, humanColor: "w" | "b"): string {
  if (state.winner === "w") return mode === "local" ? "White Wins!" : humanColor === "w" ? "You Win!" : "CPU Wins!";
  if (state.winner === "b") return mode === "local" ? "Black Wins!" : humanColor === "w" ? "CPU Wins!" : "You Win!";
  if (state.winner === "draw") return "Draw";
  if (mode === "local") return `${state.current === "w" ? "White" : "Black"} to move`;
  return state.current === humanColor ? "Your move" : "CPU...";
}
...
export function ChessGame() {
  ...
  useEffect(() => {
    if (state.winner !== null && prevStatusRef.current === null) {
      if (mode === "cpu" && state.winner === humanColor) {
        playStageClearAudio();
  ...
  const activeColor = mode === "local" ? state.current : humanColor;
  const humanTurn =
    canPlayRef.current && state.winner === null && (mode === "local" || state.current === humanColor);
  ...
  function onCell(r: number, c: number) {
    ...
    if (piece && piece[0] === color && legal.some((m) => m.from[0] === r && m.from[1] === c)) {
      setSelected([r, c]);
    } else {
      setSelected(null);
    }
  ...
  const msg = statusMessage(state, mode, humanColor);
  ...
  return (
    <div className="standard-game-shell relative flex flex-col items-center gap-4 mx-auto w-full max-w-md px-2 sm:px-0 landscape:gap-2 touch-manipulation">
      ...
      <p className="text-xs text-muted-foreground">
        {mode === "local"
          ? "같은 기기에서 흑백이 번갈아 둡니다. 캐슬링 · 앙파상 · 프로모션 지원."
          : "Castling, en passant, and pawn promotion enabled."}
      </p>
    </div>
  );
}
