...
function shardStateAgeMs(room: GameRoom | null | undefined): number {
  ...
  return roomGameStateAgeMs(room);
}
...
export function BomberGame() {
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        await joinMultiplayerRoom(code, { gameSlug: "bomber", nickname, maxPlayers: 8 });
        if (!mounted) return;
        const room = getRoom(code);
  ...
  const waitForHostStateAck = useCallback(
    (code: string, expectedMapId: number, timeoutMs = 5000): Promise<BomberHostAckWait> =>
      new Promise((resolve) => {
        ...
        const markLiveBroadcast = (room: GameRoom | null | undefined, fromStateEvent: boolean) => {
          const st = room?.gameState?.state as BomberSyncState | undefined;
          if (!fromStateEvent || !noteBomberHostState(st, expectedMapId)) return;
          if (roomGameStateAgeMs(room as GameRoom) < SHARD_STATE_STALE_MS) sawHostState = true;
        };
        ...
        const unsub = subscribeRoom(code, (r) => {
          ...
          markLiveBroadcast(r, gs._lastEvent === "state");
  ...
  const enterMapMatch = useCallback(
    (nextMapId: number) => {
      ...
      void (async () => {
        const stale = () => joinGen !== joinGenRef.current;
        try {
          if (qaLocalProbeRef.current) {
            ...
            if (hostNow) matchHostIdRef.current = deviceId;
          ...
          /* qaLocalProbeRef — isolated local grid (QA only) */
          const qaRoom = getRoom(code) ?? createRoom({
            code,
            gameSlug: "bomber",
            maxPlayers: 8,
            matchMode: "public",
        ...
        {connecting ? (
          ...
          >
            ...
            <div className="flex flex-wrap justify-center gap-2">
              ...
              <button
                type="button"
                data-testid="bomber-connect-back"
                onClick={() => {
                  window.location.href = "/games/bomber";
                }}
