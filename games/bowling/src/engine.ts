...
export function roll(state: BowlingState): BowlingState {
  ...
  const rollsInFrame = state.rollsInFrame + 1;

  if (pinsLeft <= 0) {
    const nextFrame = state.frame + 1;
    if (nextFrame > MAX_FRAMES) {
      return { ...state, score, pins: 0, lastKnock: knock, status: "over", rollsInFrame: 0 };
    }
    return {
      frame: nextFrame,
      pins: 10,
  ...
  if (state.frame >= MAX_FRAMES && rollsInFrame >= 2) {
    return { ...state, pins: pinsLeft, score, power: 0, powerDir: 1, status: "over", lastKnock: knock, rollsInFrame };
  }

  return { ...state, pins: pinsLeft, score, power: 0, powerDir: 1, status: "aiming", lastKnock: knock, rollsInFrame };
}
