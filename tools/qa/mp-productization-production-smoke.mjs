/**
 * Multiplayer Productization Production smoke — game29.vercel.app only.
 */
...
const report = {
  gate: "multiplayer-productization-production-smoke",
  baseUrl: BASE,
  approvedCommit: "4319eb2",
...
try {
  ...
  if (missing.length || leak) fail("flagship-catalog", JSON.stringify({ missing, leak }));
  ...
  for (const slug of OFFICIAL) {
    ...
    if (!/ENTER WORLD/i.test(text) || !/MULTIPLAYER/i.test(text) || practice) {
      fail(`detail-${slug}`, JSON.stringify({ href, practice }));
    }
    ...
    if (slug === "bomber") {
      ...
      if (err) {
        ...
        if (!world) fail("bomber-world", "WORLD HUD missing");
        pass("bomber-world", "HUD");
      }
